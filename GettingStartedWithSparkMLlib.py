...
# MAGIC %md
# MAGIC Because the dataset does not include column names, create a schema to assign column names and datatypes.
...
# MAGIC %md
# MAGIC What's the distribution of the number of `hours_per_week`?
...
# MAGIC %md
# MAGIC How about `education` status?
...
# MAGIC %md
# MAGIC If we wanted to use SQL, we can create a temporary view using `createOrReplaceTempView`.

# COMMAND ----------

trainDF.createOrReplaceTempView("training_data")

# COMMAND ----------

# MAGIC %sql select * from training_data
...
# MAGIC select 
# MAGIC education,
# MAGIC count(education) as count
...
# MAGIC %md
# MAGIC ### Convert categorical variables to numeric
# MAGIC 
...
# MAGIC %md
# MAGIC In this notebook, we'll build a pipeline combining all of our feature engineering and modeling steps. But let's take a minute to look more closely at how estimators and transformers work by applying the `stringIndexer` estimator that we created in the previous code block.
# MAGIC 
...
# MAGIC %md
# MAGIC ## Step 4. Build the pipeline
# MAGIC 
...
# MAGIC %md
# MAGIC Display the predictions from the model. The `features` column is a [sparse vector](https://spark.apache.org/docs/latest/api/python/pyspark.ml.html#pyspark.ml.linalg.SparseVector), which is often the case after one-hot encoding, because there are so many 0 values.
...
# MAGIC %md
# MAGIC ## Step 5. Evaluate the model
# MAGIC 
...
# MAGIC %md
# MAGIC To evaluate the model, we use the `BinaryClassificationEvaluator` to evalute the area under the ROC curve and the `MulticlassClassificationEvaluator` to evalute the accuracy.
# MAGIC 
# MAGIC For more information:  
...
# MAGIC %md
# MAGIC ## Step 7. Make predictions and evaluate model performance
...
# MAGIC %md
# MAGIC ### Use SQL queries for displaying predictions
